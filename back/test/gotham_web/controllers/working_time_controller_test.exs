defmodule GothamWeb.WorkingTimeControllerTest do
  ...
  @create_attrs %{end: "2010-04-17 14:00:00.000000", start: "2010-04-17 14:00:00.000000"}
  @update_attrs %{end: "2011-05-18 15:01:01.000000", start: "2011-05-18 15:01:01.000000"}
  @invalid_attrs %{end: nil, start: nil}

  def fixture(:working_time) do
    {:ok, working_time} = Work.create_working_time(@create_attrs)
    working_time
  end
  ...
  describe "create working_time" do
    test "renders working_time when data is valid", %{conn: conn} do
      conn = post conn, working_time_path(conn, :create), working_time: @create_attrs
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get conn, working_time_path(conn, :show, id)
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "end" => "2010-04-17T14:00:00.000000",
        "start" => "2010-04-17T14:00:00.000000"}
    end
  ...
  describe "update working_time" do
    ...
    test "renders working_time when data is valid", %{conn: conn, working_time: %WorkingTime{id: id} = working_time} do
      conn = put conn, working_time_path(conn, :update, working_time), working_time: @update_attrs
      assert %{"id" => ^id} = json_response(conn, 200)["data"]

      conn = get conn, working_time_path(conn, :show, id)
      assert json_response(conn, 200)["data"] == %{
        "id" => id,
        "end" => "2011-05-18T15:01:01.000000",
        "start" => "2011-05-18T15:01:01.000000"}
    end
  ...
  describe "delete working_time" do
    ...
    test "deletes chosen working_time", %{conn: conn, working_time: working_time} do
      ...
      assert_error_sent 404, fn ->
        get conn, working_time_path(conn, :show, working_time)
      end
    end
  end
